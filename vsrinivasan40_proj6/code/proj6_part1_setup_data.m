function imdb = proj6_part1_setup_data(methodologyNum)
...
if(methodologyNum==3 || methodologyNum==4 || methodologyNum==5)
    % Zero center all images
    mean_image = mean(imdb.images.data(:,:,1,:),4);
    mean_image = repmat(mean_image,1,1,1,size(imdb.images.data(:,:,1,:),4));
    imdb.images.data(:,:,1,:) = imdb.images.data(:,:,1,:) - mean_image;
end
