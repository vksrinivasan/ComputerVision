...
imdb.images.data   = zeros(image_size(1), image_size(2), 3, total_images, 'single'); % adjust 1 to 3 for rgb
...
for set = 1:length(sets)
    for category = 1:length(categories)
        ...
        for i = 1:length(cur_images)
            ...
            if(size(cur_image,3) < 3) % I need these to be in rgb form
...
mean_image = repmat(averageImage,1,1,1,size(imdb.images.data(:,:,1,:),4));
