...
methodologyNum = 5; % Which methodology step to get results from
...
opts.learningRate = 0.0001 ; %0.0001
...
if(methodologyNum==1)
    opts.numEpochs = 30;
elseif(methodologyNum==2)
    opts.numEpochs = 50;
elseif(methodologyNum==3)
    opts.numEpochs = 30;
elseif(methodologyNum==4)
    opts.numEpochs=100;
elseif(methodologyNum==5)
    opts.numEpochs = 75 ; 
end
...
net = proj6_part1_cnn_init(methodologyNum);
...
% else
  imdb = proj6_part1_setup_data(methodologyNum);
  imdb.methodologyNum = methodologyNum; % Just add this in here because we need it for getBatch
...
% --------------------------------------------------------------------
if(imdb.methodologyNum==1)
    im = imdb.images.data(:,:,:,batch) ;
    labels = imdb.images.labels(1,batch) ;
elseif(imdb.methodologyNum==2 || imdb.methodologyNum==3 || imdb.methodologyNum==4 || ...
        imdb.methodologyNum==5)
    ...
    % For each image we will create 4 images - mirrored, flipped, rotated
    % random amount, and zoomed in a random amount. We will then just pick one
    % from the set of original/jittered to include in the set of 50

    for ii = 1:size(im,4)

        iter_image = im(:,:,1,ii); % This iterations image
        iter_image_mir = fliplr(iter_image); % Mirror

        which_img = randi(2);
        if(which_img==1)
            im(:,:,1,ii) = im2single(iter_image);
        elseif(which_img==2)
            im(:,:,1,ii) = im2single(iter_image_mir);
        end
