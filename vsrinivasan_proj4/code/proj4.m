...
%         YOU CODE build_vocabulary.m
        if ~exist('vocab.mat', 'file')
            ...
            vocab_size = 200; % 300 - Larger values will work better (to a point) but be slower to compute
            vocab = build_vocabulary(train_image_paths, vocab_size,SIFT_size,SIFT_step_1);
            save('vocab.mat', 'vocab')
        end

        % YOU CODE get_bags_of_sifts.m

        train_image_feats = get_bags_of_sifts(train_image_paths,SIFT_size,...
            SIFT_step_2,Spatial_Pyramid,kernel_codebook_encoding);
        test_image_feats  = get_bags_of_sifts(test_image_paths,SIFT_size,...
            SIFT_step_2,Spatial_Pyramid,kernel_codebook_encoding);

        
        % Code to get GIST descriptor for each image and append it to
        % current feature histogram
        if(GIST)
            train_image_feats = get_gist_desc(train_image_paths,train_image_feats);
            test_image_feats = get_gist_desc(test_image_paths,test_image_feats);
        end
