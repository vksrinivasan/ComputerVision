...
feature_params = struct('template_size', 36, 'hog_cell_size', 6); % change back to 6

% either only 1 is flagged or they all are flagged!
linear_svm = 1;
neural_net = 0;
nearest_neighbor = 0;
...
all_examples = [features_pos;features_neg]';
labels = ones(size(all_examples,2),1);
labels(size(features_pos,1)+1:end,1) = -1;
svm_lambda = 0.0001;

[w,b] = vl_svmtrain(all_examples,labels,svm_lambda);

% now train neural net
if(neural_net==1)
    if(exist('nn_weights_20000_cs6.mat','file'))
        load('nn_weights_20000_cs6.mat');
    else
    
    target_output_train = zeros(size(all_examples',1),2);
    target_output_train(1:size(features_pos,1),1) = 1;
    target_output_train(size(features_pos,1)+1:end,2) = 1;

    target_class_train = zeros(size(all_examples',1),1);
    target_class_train(1:size(features_pos,1),1) = 1;
    target_class_train(size(features_pos,1)+1:end,1) = 2;
    weights_neural_net = train(all_examples',target_output_train,target_class_train);
    end
else
    weights_neural_net = NaN;
end

% Get KDTree for Approx. Nearest Neighbor
if(nearest_neighbor==1)
    kdtree = vl_kdtreebuild(all_examples);
else
    kdtree = NaN;
end
...
[bboxes, confidences, image_ids] = run_detector(test_scn_path, w, b, feature_params,weights_neural_net,kdtree,all_examples,labels,...
